...
if Config.UserStatisticsRanking then
    TriggerServerEvent('snowii_advanced-zombies:onPlayerStatisticsLoad')
end

RegisterNetEvent("snowii_advanced-zombies:updatePlayerStatisticsOnClient")
AddEventHandler("snowii_advanced-zombies:updatePlayerStatisticsOnClient", function(data)
    userStatistics = data
end)

RegisterNetEvent("snowii_advanced-zombies:loadedPlayerData")
AddEventHandler("snowii_advanced-zombies:loadedPlayerData", function(cb)
    loadedPlayerData = cb
end)
...
AddEventHandler('hospital:client:RespawnAtHospital', function()
    if Config.UserStatisticsRanking then
        TriggerServerEvent("snowii_advanced-zombies:updatePlayerStatistics", "deaths", 1)
    end
end)

function updatePlayerStatistics(type, count)
    if Config.UserStatisticsRanking then
        TriggerServerEvent("snowii_advanced-zombies:updatePlayerStatistics", type, count)
    end
end
...
function openUserStatisticsRankingUI()
    ...
    if loadedPlayerData then
        ...
        if Config.Framework == "ESX" then
            ...
            ESX.TriggerServerCallback('snowii_advanced-zombies:getAllPlayerStatistics', function(players)
        ...
        elseif Config.Framework == "QBCore" then
            ...
            QBCore.Functions.TriggerCallback('snowii_advanced-zombies:getAllPlayerStatistics', function(players) 
                for k,v in pairs(players) do
