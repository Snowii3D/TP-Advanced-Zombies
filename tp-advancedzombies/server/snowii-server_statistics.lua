...
RegisterServerEvent('snowii_advanced-zombies:onPlayerStatisticsLoad')
AddEventHandler('snowii_advanced-zombies:onPlayerStatisticsLoad', function()
    ...
    if loadedPlayerData then
        ...
        MySQL.Async.fetchAll('SELECT * FROM snowii_user_statistics WHERE identifier = @identifier', {
            ...
            if result[1] then
                ...
                TriggerClientEvent("snowii_advanced-zombies:updatePlayerStatisticsOnClient", _source, data)
        ...
        Wait(1000)
        TriggerClientEvent("snowii_advanced-zombies:loadedPlayerData", _source, true)

    end

end)

RegisterServerEvent('snowii_advanced-zombies:updatePlayerStatistics')
AddEventHandler('snowii_advanced-zombies:updatePlayerStatistics', function(type, count)
    ...
    if Config.UserStatisticsRanking then
        ...
        for k, v in pairs(userStatistics) do
            if v.id == identifier then
                ...
                TriggerClientEvent("snowii_advanced-zombies:updatePlayerStatisticsOnClient", _source, v)
            end
        end
